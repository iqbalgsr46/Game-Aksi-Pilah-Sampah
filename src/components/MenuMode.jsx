  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 w-full h-full flex flex-col">
        ...
        {/* ===== TOP BAR ===== */}
        <header className="w-full pt-6 short:pt-2 px-6 short:px-3 flex justify-between items-start">
          {/* Left: (Score dihapus) */}
          <div />

          {/* Right: Action Buttons (Matches MainMenu sidebar exactly) */}
          <div className="absolute top-6 short:top-2 right-6 short:right-3 flex flex-col gap-4 short:gap-2 z-10">
            {/* Tombol Keluar (Merah 3D) */}
            <motion.button 
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.9, y: 4, transition: { type: "spring", stiffness: 400, damping: 10 } }}
              className="relative overflow-hidden w-16 h-16 md:w-20 md:h-20 short:w-12 short:h-12 bg-gradient-to-b from-red-400/50 to-red-600/60 backdrop-blur-md rounded-2xl border-[4px] border-white/70 flex items-center justify-center shadow-[0_6px_0_rgba(153,27,27,0.4),inset_0_4px_8px_rgba(255,255,255,0.4)] cursor-pointer group"
              onClick={onClose}
            >
        ...
        {/* ===== MAIN CONTENT ===== */}
        <main className="flex-1 w-full flex items-center justify-between px-16 lg:px-32 mt-6 short:mt-1 relative">
          
          {/* Left Side: Character */}
          <div className="w-1/2 h-full relative flex items-end justify-center pb-24 short:pb-4">
            
          </div>

          {/* Right Side: Title & Menu Buttons */}
          <div className="w-1/2 max-w-md flex flex-col gap-6 short:gap-3 items-center mr-8 lg:mr-24 short:mr-2">
            
            {/* Title */}
            <motion.h1 
              animate={{ y: -15 }}
              transition={{ repeat: Infinity, repeatType: "mirror", duration: 2, ease: "easeInOut" }}
              className="text-6xl md:text-8xl short:text-4xl text-white mb-6 short:mb-1 text-center"
              style={{ 
            ...
            {[
              ...
              <motion.button
                ...
                className={`relative w-full py-6 px-8 short:py-3 short:px-6 rounded-[2.5rem] short:rounded-full border-[6px] short:border-[4px] border-white/50 bg-gradient-to-b ${btn.bgClass} backdrop-blur-md flex items-center justify-center gap-4 group overflow-hidden cursor-pointer`}
                style={{
                  boxShadow: `0 8px 0 ${btn.shadow}, inset 0 8px 16px rgba(255,255,255,0.4), 0 12px 20px rgba(0,0,0,0.25)`,
                }}
              >
                {/* Glossy overlay dots */}
                <div className="absolute top-2 left-6 w-5 h-5 short:w-3 short:h-3 bg-white/80 rounded-full blur-[0.5px]"></div>
                <div className="absolute top-1 left-3 w-3 h-3 short:w-2 short:h-2 bg-white/80 rounded-full blur-[0.5px]"></div>

                {/* Top Inner Curve Highlight */}
                <div className="absolute top-1 left-12 right-12 h-2 short:h-1 bg-gradient-to-b from-white/60 to-transparent rounded-full blur-[0.5px]"></div>
                
                {/* Bottom Inner Curve Highlight */}
                <div className="absolute bottom-1.5 left-10 right-10 h-1.5 short:h-1 bg-gradient-to-t from-white/50 to-transparent rounded-full blur-[0.5px]"></div>

                {/* Bottom Right Dot */}
                <div className="absolute bottom-2 right-4 w-2 h-2 short:w-1 short:h-1 bg-white/70 rounded-full blur-[0.5px]"></div>
                
                <span className="text-white text-3xl md:text-4xl short:text-2xl font-extrabold tracking-wide drop-shadow-[0_4px_0_rgba(0,0,0,0.3)] group-hover:scale-105 transition-transform relative z-10">
                  {btn.label}
                </span>
              </motion.button>
            ))}
        ...
        {/* ===== FOOTER ===== */}
        <footer className="w-full pb-4 short:pb-1 flex items-center justify-center px-4 short:hidden">
          <motion.p 
